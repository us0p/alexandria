...
| `\S`                                 | Matches any character that is not a space, as defined by the current locale. Shorthand for `[^[:space:]]`.                                                                                                                                                      |
| `\w`                                 | Matches any word-constituent character, that is, it matches any letter, digit, or underscore. Shorthand for `[[:alnum:]_]`.                                                                                                                                     |
...
| `/y` or `\b` in some implementations | Matches the empty string at either the beginning or the end of a word. For example `/\yballs?\y/` matches either `ball` or `balls`, as a separate word.                                                                                                         |
| `\B`                                 | Matches the empty string that occurs between two word-constituent characters. For example `/\Brat\B/` matches `crate`, but it does not match `dirt rat`. Another way of thinking is that `\B` matches the empty string provided it's not at the edge of a word. |
